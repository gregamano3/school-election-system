...
export default function AdminSidebar({ logoUrl }: { logoUrl: string | null }) {
  ...
  return (
    <aside className="hidden w-56 flex-col border-r border-[#dbe0e6] bg-white dark:border-[#2d394a] dark:bg-[#1a2433] lg:flex" aria-label="Admin sidebar">
      <div className="flex h-16 items-center gap-3 border-b border-[#dbe0e6] px-4 dark:border-[#2d394a]">
        <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center overflow-hidden text-white" aria-hidden="true">
          {logoUrl ? (
            <img
              src="/api/site-logo"
              alt="School logo"
            ...
            <span className="material-symbols-outlined text-3xl text-[#136dec]" aria-hidden="true">how_to_vote</span>
          )}
        </div>
